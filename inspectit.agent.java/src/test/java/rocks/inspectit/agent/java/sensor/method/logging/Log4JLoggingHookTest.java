...
//
// private static final Level[] possibleLevels = new Level[] { Level.FATAL, Level.ERROR, Level.WARN,
// Level.INFO, Level.TRACE, Level.DEBUG };
//
// @DataProvider
...
// public void checkForCorrectLogging(Level givenMinimumLevel, Level logThisLevel, Boolean
// shouldCapture) throws IdNotAvailableException {
...
//
// if (shouldCapture) {
// Mockito.verify(coreService).addMethodSensorData(eq(sensorTypeId), eq(methodId), eq((String)
// null), argThat(new LoggingDataVerifier(loggingData)));
// } else {
// Mockito.verify(coreService, never()).addMethodSensorData(eq(sensorTypeId), eq(methodId),
// eq((String) null), argThat(new LoggingDataVerifier(loggingData)));
// }
// }
//
// /**
