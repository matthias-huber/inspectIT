...
// when(sensorTypeConfig.getParameters()).thenReturn(Collections.<String, Object>
// singletonMap("forceMBeanServer", Boolean.TRUE));
//
...
//
// verify(mBeanServer).addNotificationListener(Matchers.<ObjectName> any(), captor.capture(),
// Matchers.<NotificationFilter> any(), eq(null));
...
// jmxSensor.mbeanServerAdded(mBeanServer);
//
// verify(mBeanServer).addNotificationListener(Matchers.<ObjectName> any(),
// Matchers.<NotificationListener> any(), Matchers.<NotificationFilter> any(), eq(null));
...
// MBeanAttributeInfo mBeanAttributeInfo = new MBeanAttributeInfo(testAttributeName, testAttrType,
// testAttrDescription, testAttrIsReadable, testAttrIsWriteable, testAttrIsIs);
...
// when(mBeanServer.queryNames(Matchers.<ObjectName> any(), (QueryExp)
// eq(null))).thenReturn(Collections.singleton(objectName));
...
// when(connection.analyzeJmxAttributes(eq(platformIdent), captor.capture())).thenAnswer(new
// Answer<Collection<JmxAttributeDescriptor>>() {
// @Override
// public Collection<JmxAttributeDescriptor> answer(InvocationOnMock invocation) throws Throwable {
// Collection<JmxAttributeDescriptor> descriptors = (Collection<JmxAttributeDescriptor>)
...
// jmxSensor.mbeanServerAdded(mBeanServer);
//
// JmxAttributeDescriptor sensorConfig = (JmxAttributeDescriptor)
...
// jmxSensor.mbeanServerRemoved(mBeanServer);
//
// verify(mBeanServer).removeNotificationListener(Matchers.<ObjectName> any(), captor.capture(),
// Matchers.<NotificationFilter> any(), eq(null));
// assertThat(captor.getValue().mBeanServer, is(mBeanServer));
// }
// }
...
// when(platformManager.getPlatformId()).thenReturn(platformIdent);
// when(connection.isConnected()).thenReturn(true);
// when(connection.analyzeJmxAttributes(eq(platformIdent),
// Matchers.<Collection<JmxAttributeDescriptor>> any())).thenAnswer(new
// Answer<Collection<JmxAttributeDescriptor>>() {
// @Override
// public Collection<JmxAttributeDescriptor> answer(InvocationOnMock invocation) throws Throwable {
// Collection<JmxAttributeDescriptor> descriptors = (Collection<JmxAttributeDescriptor>)
...
// verifyNoMoreInteractions(mBeanServer);
//
// ArgumentCaptor<JmxSensorValueData> valueCaptor =
// ArgumentCaptor.forClass(JmxSensorValueData.class);
// verify(coreService).addJmxSensorValueData(eq(sensorType), eq(testObjectName),
// eq(testAttributeName), valueCaptor.capture());
//
...
// assertThat(valueCaptor.getValue().getJmxSensorDefinitionDataIdentId(),
// is(equalTo(definitionDataIdentId)));
...
// when(connection.isConnected()).thenReturn(true);
// when(connection.analyzeJmxAttributes(eq(platformIdent),
// Matchers.<Collection<JmxAttributeDescriptor>>
// any())).thenReturn(Collections.<JmxAttributeDescriptor> emptyList());
// when(mBeanServer.getAttribute(objectName, testAttributeName)).thenReturn(value);
// jmxSensor.mbeanServerAdded(mBeanServer);
//
// jmxSensor.update(coreService);
...
// }
// });
// when(mBeanServer.getAttribute(objectName, testAttributeName)).thenReturn(new String[] { "1", "2",
// "3" });
// jmxSensor.mbeanServerAdded(mBeanServer);
//
// jmxSensor.update(coreService);
...
// return new Object[][] { { AttributeNotFoundException.class }, { InstanceNotFoundException.class
// }, { MBeanException.class }, { ReflectionException.class },
...
//
// jmxSensor.mbeanServerAdded(mBeanServer);
// ArgumentCaptor<MBeanServerHolder> notificationListener =
// ArgumentCaptor.forClass(MBeanServerHolder.class);
// verify(mBeanServer).addNotificationListener(Matchers.<ObjectName> any(),
// notificationListener.capture(), Matchers.<NotificationFilter> any(), eq(null));
//
// when(sensorTypeConfig.getId()).thenReturn(sensorType);
// when(mBeanServer.queryNames(Matchers.<ObjectName> any(), (QueryExp)
// eq(null))).thenReturn(Collections.singleton(objectName));
...
// });
// when(mBeanServer.getAttribute(objectName, testAttributeName)).thenReturn(value);
// MBeanServerNotification notification = new
// MBeanServerNotification(MBeanServerNotification.REGISTRATION_NOTIFICATION, this, 1, objectName);
//
...
// });
// MBeanServerNotification notification = new
// MBeanServerNotification(MBeanServerNotification.UNREGISTRATION_NOTIFICATION, this, 1,
// objectName);
//
// jmxSensor.mbeanServerAdded(mBeanServer);
// ArgumentCaptor<MBeanServerHolder> notificationListener =
